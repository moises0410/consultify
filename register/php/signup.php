                        ...
                        if (in_array($img_ext, $extensions) === true) {
                            ...
                            if (in_array($img_type, $types) === true) {
                                ...
                                if (move_uploaded_file($tmp_name, "../../uploaded_img/" . $new_img_name)) {
                                    ...
                                    $insert_query = mysqli_query($conn, "INSERT INTO users (unique_id, fname, lname, stud_id, course, year, email, password, img, status, user_type, note, data_privacy)
                            VALUES ({$ran_id}, '{$fname}', '{$lname}', '{$stud_id}', '{$course}', '{$year}', '{$email}', '{$hashed_password}', '{$new_img_name}', '{$status}', '{$user_type}', 'pending', '{$dataPrivacy}')");
                                    if ($insert_query) {
